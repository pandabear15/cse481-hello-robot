...
class InverseKinematics():
    ...
    def execute(self, target_point):
        ...
        (success, trajectories) = self.move_to_configuration(q_soln)
    ...
    def get_current_configuration(self):
        ...
        if tool == 'tool_stretch_gripper':
            ...
            print(self.Robot.readCurrentPos('joint_lift'))
            print(self.Robot.readCurrentPos('joint_arm_l0'))
            print(self.Robot.readCurrentPos('joint_arm_l1'))
            print(self.Robot.readCurrentPos('joint_arm_l2'))
            print(self.Robot.readCurrentPos('joint_arm_l3'))
            print(self.Robot.readCurrentPos('wrist_extension'))

            print(self.Robot.readCurrentPos('joint_wrist_yaw'))
            print(self.Robot.readCurrentPos('joint_wrist_pitch'))
            print(self.Robot.readCurrentPos('joint_wrist_roll'))
            
            q_lift = self.Robot.readCurrentPos('joint_lift')
            q_arml = bound_range('joint_arm_l0', self.Robot.readCurrentPos('joint_arm_l0')  / 4.0)
            q_yaw = bound_range('joint_wrist_yaw',  self.Robot.readCurrentPos('joint_wrist_yaw'))
            q_pitch = bound_range('joint_wrist_pitch',  self.Robot.readCurrentPos('joint_wrist_pitch'))
            q_roll = bound_range('joint_wrist_roll',  self.Robot.readCurrentPos('joint_wrist_roll'))
    ...
    def move_to_configuration(self, q):
        ...
        if tool == 'tool_stretch_gripper':
            ...
            lift = q[2]
            wrist_extension = q[4] + q[5] + q[6] + q[7]
            yaw = q[8]
            pitch = q[10]
            roll = q[11]

            positions = [[lift, wrist_extension, yaw, pitch, roll]]
            finishedStatus = {'min_duration': 3.0, 'end_limb': "joint_lift", 
                            'end_pos': lift, 'epsilon_offset': 0.02}
            
            trajectories = [[]]
            trajectories[0] = [joint_names, seconds, positions, finishedStatus]
